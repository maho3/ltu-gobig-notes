...
- `lcdm_z_heatmap.png` — noise-grid heatmap of mean joint Mahalanobis distance for LCDM Mnu=0 cosmologies; per point z = sqrt(dᵀ C⁻¹ d) where d = θ_true − posterior median over (Ωm, σ8) and C is the 2×2 posterior covariance of those parameters (full covariance, including the Ωm–σ8 correlation); averaged over LCDM Mnu=0 test points per noise bin; black contour marks z=2; green=well-recovered, red=biased
